...
public static class Tweener
{
    ...
    {
        while (LevelController.instance.InPlayMode && target != null && Vector3.Distance(target.localPosition, destination) > .01f)
        {
        ...
        if(LevelController.instance.InPlayMode && target != null)
    ...
    public static IEnumerator ChangeScale(Transform xForm, Vector3 targetScale, float time)
    {
        ...
        while (LevelController.instance.InPlayMode && Time.time < totalTime)
        {
            yield return null;
            xForm.localScale = Vector3.Lerp(xForm.localScale, targetScale, Time.deltaTime * speed);
        }

        if(LevelController.instance.InPlayMode)
    ...
    public static IEnumerator Rotate(Transform xForm, float angle = 25f, float time = 1f)
    {
        ...
        while (LevelController.instance.InPlayMode && Vector3.Distance(xForm.eulerAngles, targetRotation.eulerAngles) > 0.01f)
        {
